  ...
  return (
    <div style={{ minHeight: "100vh", background: "#080808", display: "flex", alignItems: "center", justifyContent: "center", padding: "80px 24px" }}>
      <div style={{ width: "100%", maxWidth: 420 }}>
        ...
        <h1 style={{ fontSize: 30, fontWeight: 900, color: "#fff", letterSpacing: "-1px", marginBottom: 8 }}>Welcome back</h1>
        <p style={{ fontSize: 14, color: "#555", marginBottom: 32 }}>Sign in to your academy dashboard.</p>
        <div style={{ background: "#0c0c0c", border: "1px solid #1a1a1a", borderRadius: 20, padding: 32 }}>
          <form onSubmit={submit} style={{ display: "flex", flexDirection: "column", gap: 18 }}>
            <div>
              <label style={{ display: "block", fontSize: 11, fontWeight: 700, color: "#555", textTransform: "uppercase", letterSpacing: ".08em", marginBottom: 8 }}>Email</label>
              <input required type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="you@academy.com" style={fi("email")} onFocus={() => setFocused("email")} onBlur={() => setFocused("")} />
            </div>
            <div>
              <label style={{ display: "block", fontSize: 11, fontWeight: 700, color: "#555", textTransform: "uppercase", letterSpacing: ".08em", marginBottom: 8 }}>Password</label>
              <input required type="password" value={password} onChange={e => setPassword(e.target.value)} placeholder="••••••••" style={fi("password")} onFocus={() => setFocused("password")} onBlur={() => setFocused("")} />
            </div>
            ...
            <p style={{ fontSize: 12, color: "#333", textAlign: "center" }}>
              No account yet?{" "}
              <Link href="/signup" style={{ color: "#60a5fa", textDecoration: "none" }}>Create your academy</Link>
            </p>
          </form>
        </div>
      </div>
