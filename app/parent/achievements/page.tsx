...
const BADGES: Badge[] = [
  { id:"first",      emoji:"🎾", title:"Первая тренировка", desc:"Посещена первая тренировка",              color:"#1F6B45", earned: s => s.attended >= 1  },
  { id:"ten",        emoji:"🔥", title:"10 тренировок",     desc:"Посещено 10 тренировок",                  color:"#e07b4f", earned: s => s.attended >= 10 },
  { id:"consistent", emoji:"💪", title:"Постоянство",       desc:"Посещено 25 тренировок",                  color:"#d97706", earned: s => s.attended >= 25 },
  { id:"fifty",      emoji:"🏆", title:"50 тренировок",     desc:"Посещено 50 тренировок",                  color:"#FFD447", earned: s => s.attended >= 50 },
  { id:"fullweek",   emoji:"📅", title:"Полная неделя",     desc:"Ни одной пропущенной тренировки на неделе", color:"#18B3A4", earned: s => s.weekNoMissed  },
  { id:"levelup",    emoji:"⬆️", title:"Новый уровень",    desc:"Ребёнок вырос выше начального уровня",    color:"#9b59b6", earned: s => s.level !== "Beginner" },
];
      ...
      <div style={{ maxWidth:720, margin:"0 auto", padding:"20px 16px", display:"flex", flexDirection:"column", gap:16 }}>
        ...
        <div style={{ background:"var(--c-card)", border:"1px solid var(--c-border)", borderRadius:16, padding:"20px" }}>
          ...
          <div style={{ height:8, background:"var(--c-inner)", borderRadius:100, overflow:"hidden" }}>
            <div style={{ width:`${(earnedCount/BADGES.length)*100}%`, height:"100%", background:"linear-gradient(90deg,#186038,#1F6B45)", borderRadius:100, transition:"width .5s" }} />
          </div>
              ...
              <div key={badge.id} style={{ background: isEarned ? "var(--c-card)" : "var(--c-inner)", border:`1px solid ${isEarned ? badge.color+"44" : "var(--c-border)"}`, borderRadius:16, padding:"20px 18px", opacity: isEarned ? 1 : 0.55, position:"relative", overflow:"hidden" }}>
                ...
                <div style={{ display:"flex", alignItems:"flex-start", gap:12 }}>
                  ...
                  <div style={{ flex:1, minWidth:0 }}>
                    ...
                    {isEarned && <span style={{ display:"inline-block", marginTop:8, fontSize:11, fontWeight:700, color:badge.color, background:badge.color+"18", padding:"2px 8px", borderRadius:100 }}>✓ Получено</span>}
                  </div>
                </div>
              </div>
            );
