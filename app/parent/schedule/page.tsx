                ...
                return (
                  <div key={di} style={{ position:"relative", borderRight: di<6?"1px solid var(--c-border)":"none", background: isT?"rgba(31,107,69,.02)":"transparent", minHeight: HOURS.length*HOUR_H }}>
                    ...
                    {daySessions.map(sess => {
                      ...
                      return (
                        <div key={sess.id}
                          style={{ position:"absolute", left:3, right:3, top: topPx+2, height: heightPx, background: meta.light, borderLeft:`3px solid ${meta.color}`, borderRadius:"0 7px 7px 0", padding:"4px 8px 4px 6px", overflow:"hidden", zIndex:3, boxShadow:`0 1px 4px rgba(0,0,0,.15)`, border:`1px solid ${meta.color}30`, borderLeftWidth:3 }}>
            ...
            return (
              <div key={i} style={{ background:"var(--c-card)", border:`1px solid ${isT?"rgba(31,107,69,.35)":"var(--c-border)"}`, borderRadius:14, overflow:"hidden" }}>
                ...
                {daySessions.map((sess, si) => {
                  const meta = SESSION_META[sess.type] || SESSION_META.Training;
                  return (
                    <div key={sess.id} style={{ padding:"12px 16px", borderBottom: si<daySessions.length-1?"1px solid var(--c-border)":"none", display:"flex", alignItems:"center", gap:12 }}>
                ...
                {daySessions.length === 0 && (
                  <div style={{ padding:"12px 16px" }}>
                    <p style={{ fontSize:12, color:"var(--c-text-dim)", margin:0 }}>{s.noSessions}</p>
                  </div>
                )}
              </div>
            );
