...
export default function Overview() {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 28 }}>
      <div>
        <h1 style={{ fontSize: 24, fontWeight: 900, color: "#fff", letterSpacing: "-1px", marginBottom: 4 }}>Overview</h1>
        <p style={{ fontSize: 14, color: "#444" }}>Your academy at a glance</p>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: 14 }}>
        {[
          ...
          <Link key={label} href={href} style={{ textDecoration: "none" }}>
            <div style={{ background: "#111", border: "1px solid #1a1a1a", borderRadius: 16, padding: 18, cursor: "pointer", transition: "border-color .2s" }}
              onMouseEnter={e => (e.currentTarget.style.borderColor = "#2a2a2a")}
              onMouseLeave={e => (e.currentTarget.style.borderColor = "#1a1a1a")}>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 10 }}>
                <p style={{ fontSize: 11, color: "#555", fontWeight: 600 }}>{label}</p>
                <span style={{ fontSize: 16 }}>{emoji}</span>
              </div>
              <p style={{ fontSize: 24, fontWeight: 900, color: "#fff", marginBottom: 4 }}>{value}</p>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
        ...
        <div style={{ background: "#111", border: "1px solid #1a1a1a", borderRadius: 16, padding: 24 }}>
          <p style={{ fontSize: 11, fontWeight: 700, color: "#555", textTransform: "uppercase", letterSpacing: ".08em", marginBottom: 16 }}>Recent Sessions</p>
          {sessions.length === 0 ? (
            <div style={{ textAlign: "center", padding: "20px 0" }}>
              <p style={{ fontSize: 13, color: "#333", marginBottom: 8 }}>No sessions logged yet</p>
              <p style={{ fontSize: 12, color: "#222" }}>Go to a player profile to log training</p>
            </div>
          ) : (
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {sessions.slice(0, 6).map(s => (
                <div key={s.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 0", borderBottom: "1px solid #161616" }}>
                  <span style={{ fontSize: 16, flexShrink: 0 }}>{SESSION_EMOJI[s.type] || "🎯"}</span>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <p style={{ fontSize: 13, color: "#ccc", fontWeight: 600, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{s.player_name}</p>
                    <p style={{ fontSize: 11, color: "#444" }}>{s.type} · {s.date}</p>
                  </div>
              ...
              {sessions.length > 6 && (
                <p style={{ fontSize: 12, color: "#333", paddingTop: 4 }}>{sessions.length - 6} more sessions</p>
              )}
            </div>
          )}
        </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          {/* Recent players */}
          <div style={{ background: "#111", border: "1px solid #1a1a1a", borderRadius: 16, padding: 24, flex: 1 }}>
            <p style={{ fontSize: 11, fontWeight: 700, color: "#555", textTransform: "uppercase", letterSpacing: ".08em", marginBottom: 14 }}>Recent Players</p>
            {players.length === 0 ? (
              <Link href="/app/players" style={{ fontSize: 13, color: "#2563eb", textDecoration: "none" }}>Add your first player →</Link>
            ) : (
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {players.slice(0, 4).map(p => (
                  <Link key={p.id} href={`/app/players/${p.id}`} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "6px 0", borderBottom: "1px solid #161616", textDecoration: "none" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                      <div style={{ width: 26, height: 26, background: "#1a1a1a", borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, fontWeight: 700, color: "#fff" }}>{p.name[0]}</div>
                      <span style={{ fontSize: 13, color: "#ccc" }}>{p.name}</span>
                    </div>
                    <span style={{ fontSize: 12, color: "#444" }}>${p.monthly_fee}/mo</span>
                  </Link>
                ))}
              </div>
            )}
          ...
          <div style={{ background: "#111", border: "1px solid #1a1a1a", borderRadius: 16, padding: 20 }}>
            <p style={{ fontSize: 11, fontWeight: 700, color: "#555", textTransform: "uppercase", letterSpacing: ".08em", marginBottom: 12 }}>Quick Actions</p>
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {[
                ...
                <Link key={label} href={href} style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 12px", background: "#0c0c0c", border: "1px solid #1a1a1a", borderRadius: 8, textDecoration: "none", color: "#666", fontSize: 13, fontWeight: 500, transition: "all .15s" }}
                  onMouseEnter={e => { const el = e.currentTarget as HTMLElement; el.style.color = "#fff"; el.style.borderColor = "#2a2a2a"; }}
                  onMouseLeave={e => { const el = e.currentTarget as HTMLElement; el.style.color = "#666"; el.style.borderColor = "#1a1a1a"; }}>
