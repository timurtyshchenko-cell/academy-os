    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
      ...
      {coaches.length === 0 ? (
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(300px, 1fr))", gap: 16 }}>
          {coaches.map(c => (
            <div key={c.id} style={{ background: "var(--c-card)", border: "1px solid var(--c-border)", borderRadius: 16, padding: 22, transition: "border-color .2s", boxShadow: "var(--c-shadow)" }}
              onMouseEnter={e => (e.currentTarget.style.borderColor = "var(--c-border-hover)")}
              onMouseLeave={e => (e.currentTarget.style.borderColor = "var(--c-border)")}>
              <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 16 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                  <div style={{ width: 44, height: 44, background: "var(--c-avatar-bg)", borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18, fontWeight: 700, color: "var(--c-avatar-text)" }}>{c.name[0]}</div>
                  <div>
                    <p style={{ fontSize: 15, fontWeight: 700, color: "var(--c-text)", marginBottom: 2 }}>{c.name}</p>
                    <p style={{ fontSize: 12, color: "var(--c-text-muted)" }}>{c.email || "No email"}</p>
                  </div>
                </div>
                <button onClick={() => deleteCoach(c.id)} style={{ padding: "6px 10px", background: "none", border: "1px solid var(--c-border)", borderRadius: 8, fontSize: 12, color: "var(--c-text-dim)", cursor: "pointer", transition: "all .15s" }}
              ...
              <div style={{ background: "var(--c-inner)", borderRadius: 8, padding: "8px 12px" }}>
                <p style={{ fontSize: 10, color: "var(--c-text-dim)", marginBottom: 2 }}>Specialty</p>
                <p style={{ fontSize: 13, color: "var(--c-text-3)" }}>{c.specialty || "General"}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      {showAdd && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.7)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, padding: 24 }}>
          <div style={{ background: "var(--c-card)", border: "1px solid var(--c-border)", borderRadius: 20, padding: 32, width: "100%", maxWidth: 440 }}>
            <h2 style={{ fontSize: 18, fontWeight: 800, color: "var(--c-text)", marginBottom: 24 }}>Add Coach</h2>
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              ...
              <div>
                <label style={{ display: "block", fontSize: 11, fontWeight: 700, color: "var(--c-text-muted)", textTransform: "uppercase", letterSpacing: ".08em", marginBottom: 6 }}>Specialty</label>
