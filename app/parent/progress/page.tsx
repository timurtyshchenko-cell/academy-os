...
export default function ParentProgress() {
  ...
  const withNotes = sessions.filter(s => s.notes).length;
    ...
    <div style={{ minHeight:"100vh", background:"var(--c-bg)", fontFamily:"inherit" }}>
      ...
      <div style={{ background:"linear-gradient(135deg,#186038,#1F6B45)", padding:"18px 24px", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
        <div style={{ display:"flex", alignItems:"center", gap:12 }}>
          ...
          <div>
            ...
            <p style={{ fontSize:16, fontWeight:800, color:"#fff", margin:0 }}>Прогресс · {playerName}</p>
          </div>
        </div>
      ...
      <div style={{ maxWidth:720, margin:"0 auto", padding:"20px 16px", display:"flex", flexDirection:"column", gap:16 }}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:10 }}>
          {[
            { label:"Всего", value: sessions.length, color:"#1F6B45" },
            { label:"Посещено", value: attended, color:"#18B3A4" },
            { label:"Пропущено", value: missed, color:"#ef4444" },
            { label:"Часов", value: totalHours.toFixed(1), color:"#d97706" },
          ].map(st => (
        ...
        {sessions.length === 0 ? (
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
            {sessions.map(sess => {
              ...
              const attended = sess.attendance !== "missed";
              return (
                <div key={sess.id} style={{ background:"var(--c-card)", border:"1px solid var(--c-border)", borderRadius:14, overflow:"hidden" }}>
                  ...
                  <div style={{ display:"flex", alignItems:"center", gap:12, padding:"14px 16px", borderBottom: (sess.notes || sess.rating) ? "1px solid var(--c-border)" : "none" }}>
                    ...
                    <div style={{ flex:1, minWidth:0 }}>
                      <div style={{ display:"flex", alignItems:"center", gap:8, flexWrap:"wrap" }}>
                        ...
                        <span style={{ fontSize:11, fontWeight:700, color: attended ? "#1F6B45" : "#ef4444", background: attended ? "rgba(31,107,69,.1)" : "rgba(239,68,68,.1)", padding:"2px 8px", borderRadius:100 }}>
                          {attended ? "✓ Посещено" : "✗ Пропущено"}
                        </span>
                      </div>
                  ...
                  {sess.notes && (
                    <div style={{ padding:"12px 16px", background:"var(--c-inner)" }}>
                      <p style={{ fontSize:11, fontWeight:700, color:"var(--c-text-muted)", textTransform:"uppercase", letterSpacing:".07em", margin:"0 0 4px" }}>Заметка тренера</p>
